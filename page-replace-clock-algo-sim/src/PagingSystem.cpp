 ...
 *
 * Return an int of the current memory size
 *
 * @returns int Returns an integer representing the memory size.
 */
 ...
 *
 * @returns int Returns an integer representing the current system time.
 */
...
/**
 * @brief get number page references
 *
 * Return the number of page references
 *
 * @returns int Returns an integer representing the page reference.
 */

int PagingSystem::getNumPageReferences() const
{
  return this->numPageReferences;
}

/**
 * @brief is memory full
 *
 * Return true if no frame is currently an empty frame, otherwise return false
 *
 * @returns bool Returns a flag representing the state of memory either being full or not.
 */

bool PagingSystem::isMemoryFull() const
{
  // Iterate over array
  for (int i = 0; i < this->getMemorySize(); i++)
  {
    // If index is equal to EMPTY_FRAME (0), return false
    if (this->memory[i] == EMPTY_FRAME)
    {
      return false;
    }
  }
  // All frames are not empty so return true
  return true;
}
 ...
 *
 * @returns bool Returns a boolean flag representing if the referenced page is in memory or not.
 */
...
bool PagingSystem::isPageHit() const
{
  ...
  for (int i = 0; i < this->getMemorySize(); i++)
  {
    // if reference page found in memory return true
    if (this->memory[i] == this->pageReference[this->systemTime])
    {
      return true;
    }
  }
  // No frame has reference page loaded so return false
  return false;
}

/**
 * @brief do page placement
 *
 * Finds the first empty frame and replaces it with current page reference.
 *
 * @returns void
 */
...
void PagingSystem::doPagePlacement()
{
  ...
  {
    stringstream msg;
    msg << "<PagingSystem::doPagePlacement> Error: current memory is full" << endl;
    throw SimulatorException(msg.str());
  }
  ...
  for (int i = 0; i < this->getMemorySize(); i++)
  {
    // If frame is empty, replace with current page reference
    if (this->memory[i] == EMPTY_FRAME)
    {
      this->memory[i] = this->pageReference[this->systemTime];
      break;
    }
  }
}
