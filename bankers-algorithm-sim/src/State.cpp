 ...
 * @brief needs are met
 *
 * Checks the given process ID resource needs versus resources currently available
 * to determine if the process needs are met.
 *
 * @param process_id A process_id/index to check what it's needs are.
 *
 * @param currentResourcesAvailable an array of integers representing the current
 *  number of resources available fo reach resource type.
 *
 * @returns False if the resource need is greater than current available resources or True
 *  if the needs is less than or equal to the current available resources.
 */
bool State::needsAreMet(int process_id, int currentResourcesAvailable[]) const
{
  // for each column (or resource) check if the need of the process_id is greater than current resources
  // avaialable. If it is greater then return false otherwise return true.
  for (int col = 0; col < this->numResources; col++)
  {
    if (this->need[process_id][col] > currentResourcesAvailable[col])
    {
      return false;
    }
  }
...
/**
 * @brief find candidate process
 *
 * Checks if needs are met for any process that is running. Returns the first process_id found
 * that has its needs met, or NO_CANDIDATE if no needs are met.
 *
 * @param  completed_work an array of boolean flags to
 *  indicate whether or not a process has completed its work yet.
 *
 * @param currentResourcesAvailable an array of integers representing the current
 *  number of resources available fo reach resource type.
 *
 * @returns process_id of first candidate who's needs can be met by
 *  currently available resources. return defined constant NO_CANDIDATE if no process
 *  can meet the needs of available resources.
 */
int State::findCandidateProcess(bool completed_work[], int currentResourcesAvailable[]) const
{
  // iterate over all processes and check if the needs are met
  // the process is not compelted and the needs are met then return that process id
  // else return our const NO_CANDIDATE at end of loop
  for (int process_id = 0; process_id < this->numProcesses; process_id++)
  {
    bool process_needs_met = needsAreMet(process_id, currentResourcesAvailable);

    if (completed_work[process_id] == false and process_needs_met)
    {
      return process_id;
    }
  }
 ...
 *
 * Returns currently allocated resources to back to the currently available resources.
 *
 * @param process_id A process_id/index to check what it's needs are.
 *
 * @param currentResourcesAvailable an array of integers representing the current
 *  number of resources available fo reach resource type.
 */
void State::releaseAllocatedResources(int process_id, int* currentResourcesAvailable) const
{
  // for each column (or resource) add the allocated resources back to
  // the current resource available
  for (int col = 0; col < this->numResources; col++)
  {
    currentResourcesAvailable[col] += this->allocation[process_id][col];
  }
}
 ...
 * @brief is safe
 *
 * Checks if the provided state is safe or not. if the completed process all return true
 * then the isSafe() function returns true showing that the state is safe. If any process returns false
 * then the state is not safe and the function returns false.
 *
 * @returns true if the state is safe, or false is state is not safe
 */
bool State::isSafe() const
{
  // Create copy of resources available vector
  int resourcesAvailableCopy[MAX_RESOURCES];
  copyVector(this->numResources, this->resourceAvailable, resourcesAvailableCopy);

  // Set all values of completed[] to false so all processes are running
  bool completed[MAX_PROCESSES];
  ...
  // Initiliaze candidate process, continue loop if not equal to NO_CANDIDATE
  int candidateProcess = 0;
  while (candidateProcess != NO_CANDIDATE)
  {
    // find a candidate process by calling finCandidateProcess() and providing
    // completed[] and copied resources vector
    candidateProcess = findCandidateProcess(completed, resourcesAvailableCopy);
    if (candidateProcess != NO_CANDIDATE)
    {
      // if candidate process isn't equal to NO_CANDIDATE then release the allocated resources
      // back to our copied vector
      releaseAllocatedResources(candidateProcess, resourcesAvailableCopy);
      // Set candidate process to True

      completed[candidateProcess] = true;
    }
  }

  // if any completed process is False, we return false, otherwise return true
  for (int i = 0; i < this->numProcesses; i++)
  {
