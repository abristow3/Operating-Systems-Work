...
public:
  ...
  bool needsAreMet(int process_id, int current_resources_available[]) const;
  int findCandidateProcess(bool completed_work[], int current_resources_available[]) const;
  void releaseAllocatedResources(int process_id, int* current_resources_available) const;
  bool isSafe() const;
